...
object PageRankT3{
    def main(args: Array[String]) {
        val spark = SparkSession.builder.appName("Page Rank Task 3").getOrCreate()

        println(s"Page rank task 3 running")
        ...
        val data = lines.map{ s =>
            ...
            (parts(0), parts(1))
        }.distinct().groupByKey().partitionBy(new HashPartitioner(10)).cache()
        ...
        ranks.saveAsTextFile("hdfs://128.104.222.128:9000/user/output/pr3")
